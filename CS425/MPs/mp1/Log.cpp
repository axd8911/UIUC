...
void Log::logHeartBeat(Address *thisNode, Address *addedAddr) {
    static char stdstring[100];
    sprintf(stdstring, "Node %d.%d.%d.%d:%d HeBeat at time %d", addedAddr->addr[0], addedAddr->addr[1], addedAddr->addr[2], addedAddr->addr[3], *(short *)&addedAddr->addr[4], par->getcurrtime());
    LOG(thisNode, stdstring);
}

void Log::logString(Address* thisNode, string str){
    static char stdstring[100];
    int p = 0;

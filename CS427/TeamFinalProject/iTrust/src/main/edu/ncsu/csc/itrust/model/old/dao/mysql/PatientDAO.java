...
public class PatientDAO {
		...
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = patientLoader
						.loadParameters(conn.prepareStatement("UPDATE patients SET firstName=?,lastName=?,email=?,"
								...
								+ "AlternateName=?, DateOfDeactivation=?,  obstetricEligible=? WHERE MID=?"), p)) {
	...
	public int updateRhShotIndicator(long patientMID,byte shotTaken) throws DBException {
		try (Connection conn = factory.getConnection();
			 PreparedStatement ps = conn.prepareStatement("UPDATE patients set rh_shot_ind=? WHERE MID = ?;")) {
			ps.setLong(1, shotTaken);
			ps.setLong(2, patientMID);
			int updated = ps.executeUpdate();
			return updated;
		} catch (SQLException e) {
